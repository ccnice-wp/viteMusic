...
let { playList,recentlyList, player, playIndex } = storeToRefs(playInfoStore)

const recentlyListReverse = ref([])
...
onMounted(() => {
  ...
  for(let i = 0; i < recentlyList.value.length; i++) {
    recentlyListReverse.value.push(recentlyList.value[i])
  }
  recentlyListReverse.value.reverse()

  for(let i = 0; i < recentlyListReverse.value.length; i++) {
    //获取音乐时长
    let duration
    let m
    let s
    ...
    //生成一个html标签
    let audioElement = new Audio(recentlyListReverse.value[i].url);
...
const playMusic = (value) => {
  playList.value = recentlyListReverse.value
...
<div>
    ...
    <div style="height: 550px;" class=" ml-6 mr-6 mt-8 space-y-2 ">
      ...
      <div style="height: 480px;" class="flex-wrap overflow-x-hidden overflow-y-scroll">
        ...
        <div v-for="(item, key) in recentlyListReverse" :key="key">
